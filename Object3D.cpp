...
void Object3D::defaultSettings()
{
	...
	diffuseColor=Vector4D(0.5,0.5,0.5,1);
}
...
void Object3D::Draw ()
{
	...
	if( !selected )
	{
		...
		glMaterialfv(GL_FRONT_AND_BACK,GL_AMBIENT_AND_DIFFUSE,diffuseColor.Array());
	}
	else
	{
	...
	glMaterialfv(GL_FRONT_AND_BACK,GL_SPECULAR,diffuseColor.Array());
		...
		switch( Type )
	{
		...
		//dodec wireframe
		case Dodec : glutWireDodecahedron(); break;
		//piramida
		case Piramida : glutWireTetrahedron(); break;
		// tor wireframe
		case Tor: glutWireTorus(0.5,1,levelOfDetail,levelOfDetail); break;
		// lampa wireframe
		...
		switch( Type )
	{
		...
		//dodec solid
		case Dodec : glutSolidDodecahedron(); break;
		//piramida
		case Piramida : glutSolidTetrahedron(); break;
		// tor solid
		case Tor: glutSolidTorus(0.5,1,levelOfDetail,levelOfDetail); break;
		// lampa wireframe
...
void Object3D::SetDiffuseColor(Vector4D c)
{
	diffuseColor.x=c.x;
	diffuseColor.y=c.y;
	diffuseColor.z=c.z;
...
void Object3D::SetBox(float s,float j,float st,float dr)
{
	box.sus=s;
	box.jos=j;
	box.stanga=st;
...
//functie care verifica coliziunea dintre obiectul asupra careia se aplica s obiectul dat ca parametru
bool Object3D::Colision(Object3D* o)
{
	if(this->box.jos+this->GetPosition().z<o->box.sus+o->GetPosition().z)
		return false;
	if(this->box.sus+this->GetPosition().z>o->box.jos+o->GetPosition().z)
		return false;
	if(this->box.stanga+this->GetPosition().x>o->box.dreapta+o->GetPosition().x)
		return false;
	if(this->box.dreapta+this->GetPosition().x<o->box.stanga+o->GetPosition().x)
		return false;
